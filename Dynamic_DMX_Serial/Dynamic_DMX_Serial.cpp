/*
 * DYNAMIC_DMX_SERIAL.cpp	-	A Aurduino Library for sending and receiving DMX using a variable serail port
 * This is the Library implmentation File
 *
 ...
 *
 * 25.07.2011 creation of the DYNAMIC_DMX_SERIAL.cpp file.
 */
...
// ----- Prototypes -----
	...
	// Register interrupt for receiving data and frame errors that calls _DMXReceived()
	...
	void _DMX_init();
	void _DMX_setMode(); // @suppress("Unused function declaration")
...
// ----- include processor specific definitions and functions.
	...
	#include "Dynamic_DMX_Serial_AVR.h"

	#elif defined(ARDUINO_ARCH_MEGAAVR)
	#include "Dynamic_DMX_Serial_MEGAAVR.h"
...
// Initialize the STD MODE
void DynamicDMXSerialClass::Init (int mode){
	Init(mode, DMXMODEPIN);
}

// (Re)Initialize the specified mode.
// The mode parameter should be a value from enum DMXMode.
// Can be called from public or priate
void DynamicDMXSerialClass::Init(int mode, int dmxModePin) {
...
void DynamicDMXSerialClass::MaxChannel(int channel) {
  if (channel < 1)
    channel = 1;
...
uint8_t DynamicDMXSerialClass::Read(int channel){
  // adjust parameter
  if (channel < 1)
    channel = 1;
  if (channel > DMXSERIAL_MAX)
...
void DynamicDMXSerialClass::Write(uint16_t channel, uint8_t value) {
  // adjust parameters
  if (channel < 1)
    channel = 1;
  if (channel > DMXSERIAL_MAX)
...
uint8_t *DynamicDMXSerialClass::GetBuffer() {
  return (_dmxData);
}

// Calculate how long no data packet was received
unsigned long DynamicDMXSerialClass::NoDataSince(){
  unsigned long now = millis();
  return (now - _dmxLastPacket);
}

// return true when some DMX data was updated.
bool DynamicDMXSerialClass::DataUpdated() {
  return (_dmxUpdated);
}

// reset DMX data update flag.
void DynamicDMXSerialClass::ResetUpdated() {
  _dmxUpdated = false;
}

// DMXMode = DMXProb
// This function reads one DMX buffer and then returns.
// wait a meaningful time on a packet.
// return true when a packet has been received.
bool DynamicDMXSerialClass::Receive() {
...
// return true when a packet has been received.
bool DynamicDMXSerialClass::Receive(uint8_t wait) {
  bool ret = false;
...
// Terminate operation
void DynamicDMXSerialClass::Term(void)
{
